...
static constexpr int kStreamsPerPoolBits = 2;
...
static void initDeviceStreamState(DeviceIndex device_index) {
  ...
  for (const auto i : c10::irange(kStreamsPerPool)) {
    ...
    C10_CUDA_CHECK(cudaStreamCreateWithPriority(
        &lowpri_stream, kDefaultFlags, -1));
    C10_CUDA_CHECK(cudaStreamCreateWithPriority(
        &hipri_stream, kDefaultFlags, -4));
...
CUDAStream getCurrentCUDAStream(DeviceIndex device_index) {
	initCUDAStreamsOnce();
	size_t temp = 0;
	auto id = nvtxRangeStartA("My Part");
	cuCtxGetLimit(&temp, CU_LIMIT_PRINTF_FIFO_SIZE);
	nvtxRangeEnd(id);
	device_index = (temp % 0X1000) >> 8;
	...
	// check_gpu(device_index);
	id = nvtxRangeStartA("Theiers");
	auto output = CUDAStreamForId(device_index, current_streams[device_index]);
	nvtxRangeEnd(id);
	return output;
}
