...
{
	modules.push_back(MySequential(Conv2d(conv_options(in_channels, out_channels, 1, 1, 0, 1, false)),
		BatchNorm2d(BatchNorm2dOptions(out_channels)),
		ReLU()));
	...
	for (int i = 0; i < modules.size(); i++)
		register_module("aspp" + to_string(i), modules[i]);
...
torch::Tensor ASPPImpl::forward(torch::Tensor x)
{
	...
	for (int i = 0; i < modules.size(); i++)
		res.push_back(modules[i]->forward(x));
	...
	vector<int> atrous_rates, int output_stride)
{
	...
	up = Upsample(UpsampleOptions().align_corners(true).scale_factor(vector<double>({ scale_factor,scale_factor })).mode(torch::kBilinear));
