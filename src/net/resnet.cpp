...
struct BasicBlock : public MyModule
{
	...
	{
		addOperation("conv1", m_conv1.ptr());
		addOperation("bn1", m_bn1.ptr());
		addOperation("relu", m_relu.ptr());

		addOperation("conv2", m_conv2.ptr());
		addOperation("bn2", m_bn2.ptr());

		if (!m_downsample->is_empty())
		{
			m_downsample.assignOperations();
			addOperations("downsample", m_downsample.getOperations());
		}
...
struct Bottleneck : public MyModule
{
	...
	void assignOperations()
	{
		...
		addOperation("conv3", m_conv3.ptr());
		addOperation("bn3", m_bn3.ptr());
...
template <typename Block, typename = std::enable_if_t<std::is_base_of<MyModule, Block>::value>>
struct ResNet : public MyModule
{
													 ...
													 int64_t stride = 1, bool dilate = false)
	{
		...
		layers.addOperations("block1", block.getOperations());
		...
		for (int64_t i = 0; i < blocks; i++)
		{
			...
			layers->push_back(block);
			block.assignOperations();
			layers.addOperations("block" + to_string(i + 2), block.getOperations());
		}
	...
	void assignOperations()
	{
		...
		addOperation("bn1", m_bn1.ptr());
		addOperation("relu", m_relu.ptr());
		addOperation("maxpool", m_maxpool.ptr());
		cout << "Count till now: " << getOperations().size() << endl;

		addOperations("layer1", m_layer1.getOperations());
		cout << "Count till now: " << getOperations().size() << endl;
		addOperations("layer2", m_layer2.getOperations());
		cout << "Count till now: " << getOperations().size() << endl;
		addOperations("layer3", m_layer3.getOperations());
		cout << "Count till now: " << getOperations().size() << endl;
		addOperations("layer4", m_layer4.getOperations());
		cout << "Count till now: " << getOperations().size() << endl;

		addOperation("avgpool", m_avgpool.ptr());
		addOperation("fc", m_fc.ptr());
		cout << "Count till now: " << getOperations().size() << endl;
	}
};
